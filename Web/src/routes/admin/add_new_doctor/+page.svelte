<script lang="ts">
    ...
    async function submitForm() {
        ...
        try {
            const response = await fetch('http://localhost:7999/api/newDoctor', {
                method: 'POST',
            ...
            if (response.ok) {
                message = `醫生資料登記成功！醫師編號：${data.doctor_id}`;
                name = '';
                birth = '';
                phone = '';
                address = '';
...
<main>
    ...
    <div class="form-container">
        <div class="field">
            <label for="name">醫生姓名 *</label>
            <input id="name" type="text" bind:value={name} placeholder="請輸入醫生姓名" required />
        </div>
        ...
        <div class="field">
            <label for="history">學歷或過往經歷</label>
            <textarea id="history" rows="5" bind:value={history} placeholder="學歷、就業經歷、個人經歷等等（直接填入或填無）"></textarea>
        </div>
